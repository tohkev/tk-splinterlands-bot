...
async function getPlayerBattlesHistory(player = "") {
	return await fetch(
		"https://api.steemmonsters.io/battle/history?player=" +
			player +
			"&v=1582143214911&token=5C9VPKBVV4&username=" +
			process.env.ACCOUNT.split("@")[0]
	)
		.then((response) => {
			if (!response.ok) {
...
const makeInsertQuery = (battleDetails) =>
	"INSERT INTO battles (summoner_id, summoner_level, monster_1_id, monster_1_level,monster_1_abilities, monster_2_id, monster_2_level, monster_2_abilities, monster_3_id, monster_3_level, monster_3_abilities, monster_4_id, monster_4_level, monster_4_abilities, monster_5_id, monster_5_level, monster_5_abilities, monster_6_id, monster_6_level, monster_6_abilities, created_date, match_type, mana_cap, ruleset, inactive, battle_queue_id, player_rating_initial, player_rating_final, winner) VALUES('" +
	Object.values(battleDetails).join("','") +
	"') ON CONFLICT (battle_queue_id) DO NOTHING;";

async function writeDB() {
	await pool.connect();
	await getPlayerBattlesHistory(process.env.ACCOUNT.split("@")[0])
		.then((result) => {
			log("TOTAL ROWS: ", result.length);
			result.map(async (battle) => {
				if (extractBattleDetails(battle)) {
					log(makeInsertQuery(extractBattleDetails(battle)));
					await pool
						.query(makeInsertQuery(extractBattleDetails(battle)))
						.then((dbResult) => log(dbResult.rowCount))
