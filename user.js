...
const isRented = (username, card) => {
	if (
		...
		(card.last_used_player === username ||
			card.last_used_player === null ||
			(card.last_used_player !== username &&
				moment.duration(moment().diff(card.last_used_date)).days() > 1))
	) {
		// not locked
		return true;
	} else {
...
getPlayerCards = (username) =>
	fetch(`https://api2.splinterlands.com/cards/collection/${username}`, {
		...
		.then((x) =>
			x["cards"]
				? x["cards"]
						.filter((x) => isPlayable(username, x))
